...
pub fn fetch_stock(symbol: &str) -> Result<StockData, String> {
    ...
    // One quote request per symbol keeps a 20-item refresh within common
    // Finnhub rate limits. The old implementation also fetched a company
    // profile every time, doubling requests for a cosmetic display name.
    ...
    let quote: FinnhubQuote = crate::http::get_json(
        "Finnhub quote",
        client.get(quote_url).header("Accept", "application/json"),
    )?;

    if !quote.c.is_finite() || quote.c <= 0.0 {
        return Err("Finnhub returned no current price for this symbol".into());
    }
